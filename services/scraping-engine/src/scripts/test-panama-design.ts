...
async function testPanamaDesign() {
  console.log('🎯 Test specifico per Panama Design')
  ...
  try {
    ...
    console.log(`  Caricamento lento: ${analysis.issues.slowLoading}`)
    ...
    console.log('\n🧮 CALCOLO SCORE:')
    let scoreBreakdown = 100
    if (analysis.issues.missingTitle) { scoreBreakdown -= 15; console.log('  -15 per titolo mancante') }
    if (analysis.issues.missingMetaDescription) { scoreBreakdown -= 10; console.log('  -10 per meta description mancante') }
    if (analysis.issues.missingH1) { scoreBreakdown -= 10; console.log('  -10 per H1 mancante') }
    ...
    if (analysis.issues.noCookieConsent) { scoreBreakdown -= 15; console.log('  -15 per nessun cookie consent') }
    if (analysis.issues.missingPartitaIva) { scoreBreakdown -= 20; console.log('  -20 per P.IVA mancante') }
